...
export class DefaultHeaderComponent {
  ...
  readonly languageModes = [
    { name: 'vi', text: 'Tiếng việt', icon: 'cifVn' },
    { name: 'en', text: 'English', icon: 'cifGb' },
    { name: 'id', text: 'Bahasa Indonesia', icon: 'cifId' }
  ];
  title = "";
  ...
  changeLanguageMode(mode: string) {
    ...
    this.switchLanguageAnt(mode);
    this.languageIcon = this.languageModes.find(d => d.name === mode)?.icon ?? 'cifVn';
  }

  switchLanguageAnt(event) {
    console.log(event);
    // if (event == 'vi')
